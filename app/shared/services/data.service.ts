...
export class DataService {
    ...
    getSchedules(page?: number, itemsPerPage?: number): Observable<PaginatedResult<ISchedule[]>> {
        ...
        return this.http.get(this._baseUrl + "schedules", { headers: headers})
            .map((res: Response) => {
    ...
    updateSchedule(schedule: ISchedule): Observable<void> {
        ...
        return this.http.put(this._baseUrl + "schedules/" + schedule.id, JSON.stringify(schedule), { headers: headers})
            .map((res: Response) => {
                return;
            }).catch(this.handleError);
    }
